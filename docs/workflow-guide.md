...
Esta guia explica como se opera el workflow real de este template sin obligarte a reconstruirlo leyendo skill por skill.
...
| `/sdd:new <nombre>` | Abrir un change completo | Recorre `propose -> spec -> design` como gate tecnico -> `tasks` |
...
| `/sdd:patch` | Resolver un cambio chico con trazabilidad | Usa un unico `patch.md`, implementa y archiva |
...
Hay una diferencia importante entre interfaz publica y fases internas:
...
- En este template no se presentan `spec`, `design` ni `tasks` como comandos publicos separados.
...
- `/sdd:domain-brief` no es una fase del change: es una utilidad publica aparte.
...
| `sdd-propose` | Definir el problema, alcance y motivacion |
| `sdd-spec` | Convertir la propuesta en requirements y escenarios |
| `sdd-design` | Decidir si hace falta diseno tecnico antes de planificar tareas |
| `sdd-tasks` | Bajar la spec a trabajo ejecutable |
| `sdd-apply` | Implementar las tareas y actualizar el estado |
| `sdd-verify` | Contrastar implementacion, specs y evidencia |
...
|------|--------------|----------------|
| `openspec/config.yaml` | Lo escribe `sdd-init` | Configuracion file-based del workflow |
...
| `openspec/changes/{change-name}/design.md` | `sdd-design` cuando aplica | Puede no existir si el gate tecnico concluye que no hace falta |
...
| `openspec/changes/patch-.../patch.md` | `sdd-patch` | Documento unico del atajo de patch |
| `docs/domain-brief.md` | `domain-brief` | No se crea vacio en `init`; se genera cuando ya hay specs consolidadas |

## 4. Configuracion que importa

`/sdd:init` explica cada decision en el momento en que la propone, pero conviene conocer estos conceptos base:
...
| `modules.skill_registry` | `true` | Si fases tecnicas pueden resolver skills del proyecto |
...
- El routing por fase importa sobre todo cuando el editor soporta delegacion y `modules.model_routing` esta activado.
...
- `domain` se usa como agrupacion conceptual o explicacion funcional, por ejemplo en `domain-brief`.
...
2. `docs/workflow-guide.md` para la interfaz publica, el flujo real y los artefactos.
